...
class TestExplorePage:
    ...
    def test_explore_page(self, setup, login_direct_complete, logger, test_config):
        ...
        assert experimental_data_tab.is_displayed(), "Experimental data tab is not displayed"
        ...
        ls_info = explore_page.get_local_storage_species_info()
        for key, val in ls_info.items():
            logger.info(f"  localStorage {key}: {val}")
        ...
        if species_has_density:
            ...
            perf_entries = explore_page.get_slow_network_resources()
            if perf_entries:
            ...
            logger.info("Found the switch count/density button")
            ...
            logger.info(f"Switch toggled from '{current_state}'.")
        ...
        # Verify experimental data sections are present
        logger.info("Searching for Experimental Data types")
        missing_locators, not_displayed = explore_page.verify_experimental_data_sections(timeout=5)
        ...
        # Verify Explore Page tab buttons
        logger.info("Searching for Explore Page titles")
        explore_page_titles = explore_page.verify_explore_page_tab_buttons(timeout=15)
        for page_title in explore_page_titles:
            assert page_title.is_displayed(), f"Explore page title {page_title} is not displayed"
        ...
        logger.info("Brain region panel search field is found")
        ...
        # Verify model data sections
        logger.info("Searching for Model data artifact titles")
        missing_locators, not_displayed = explore_page.verify_model_data_sections(timeout=5)
        assert not missing_locators and not not_displayed, (
