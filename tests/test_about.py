...
class TestAbout:
    def test_about(self, visit_public_pages, logger):
        ...
        # load_more = about_page.find_load_more_btn()
        # assert load_more.is_displayed(), "Load more button is not displayed"
        # logger.info("Load more button is visible")

        # click_load_more_btn = about_page.click_load_more_btn(timeout=10)
        # browser.execute_script("arguments[0].scrollIntoView({block: 'center'});", click_load_more_btn)
        # logger.info("Found the Load more button that is clickable")
        # click_load_more_btn.click()
