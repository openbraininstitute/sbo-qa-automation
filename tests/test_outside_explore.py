...
class TestOutsideExplorePage:
    ...
    def test_outside_explore_page(self, setup, login, logger, test_config):
        ...
        outside_explore_page = OutsideExplorePage(browser, wait, base_url)
        print(f"DEBUG: Using lab_id={lab_id}, project_id={project_id}")
        outside_explore_page.go_to_outside_explore_page()
        logger.info(f"Explore page is loaded, {browser.current_url}")

        outside_explore_page.check_explore_title_is_present()
        logger.info("Explore page title is present")

        cerebrum_title = outside_explore_page.cerebrum_title()
        ...
        exp_data_elements = outside_explore_page.find_experimental_data_titles(exp_data_titles)
        ...
        explore_page_titles = outside_explore_page.find_explore_page_titles(page_titles)
        ...
        record_counts = outside_explore_page.get_experiment_record_count(record_count_locators)
        ...
        brain_region_panel = outside_explore_page.find_brain_region_panel()
        logger.info("Found Brain Region Panel")

        cerebrum_in_brpanel = outside_explore_page.find_cerebrum_brp()
        logger.info("Found Cerebrum in the brain region panel")

        cerebrum_arrow_btn = outside_explore_page.find_cerebrum_arrow_btn()
        logger.info("Cerebrum - parent arrow button is clicked")
        browser.execute_script("arguments[0].click();", cerebrum_arrow_btn)

        cerebral_cortex_title = outside_explore_page.find_cerebral_cortex_brp()
        logger.info("Found Cerebral cortex as a child of Cerebrum")

        neurons_panel = outside_explore_page.find_neurons_panel()
        assert neurons_panel.is_displayed()
        logger.info("Neurons panel is displayed")

        density_count_switch = outside_explore_page.find_count_switch()
        assert density_count_switch.is_displayed()
        logger.info("Density & count switch is displayed")

        brain_region_search_field = outside_explore_page.find_brain_region_search_field(timeout=25)
        ...
        outside_explore_page.wait_for_long_load(find_input_file_and_wait)
        logger.info("Waiting for page to load")

        brain_region_search_field.send_keys("Isocortex")
        logger.info("Searching for 'Isocortex'")
        brain_region_search_field.send_keys(Keys.ENTER)
        selected_brain_region_title = outside_explore_page.find_selected_brain_region_title()
        ...
        outside_explore_page.wait_for_page_ready(timeout=20)
        logger.info("Wait for the sorting action to complete.")
        model_data_tab = outside_explore_page.find_model_data_title()
        ...
        expected_panel = outside_explore_page.find_data_panel()
        ...
        panel_emodel = outside_explore_page.find_panel_emodel()
        logger.info("E-model is found in the types panel")

        panel_memodel = outside_explore_page.find_panel_memodel()
        logger.info("ME-model is found in the types panel")

        panel_synaptome = outside_explore_page.find_panel_synaptome()
        logger.info("Synaptome is found in the types panel")

        liteture_tab = outside_explore_page.literature_title().click()
        ...
        literature_panel = outside_explore_page.find_literature_panel_data(literature_panel_data_titles)
        ...
        atlas = outside_explore_page.find_3d_atlas()
        assert atlas.is_displayed()
        logger.info("3D Atlas is displayed")

        atlas_fullscreen = outside_explore_page.find_atlas_fullscreen_bt(timeout=15)
        logger.info("Found atlas fullscreen button")
        atlas_fullscreen.click()

        fulscreen_exit = outside_explore_page.find_fullscreen_exit(timeout=15)
        ...
        total_count_density_title = outside_explore_page.find_total_count_density()
        ...
        total_count_number = outside_explore_page.find_total_count_n()
        ...
        count_switch_button = outside_explore_page.find_total_count_switch()
        ...
        neuron_panel_one_mtype = outside_explore_page.find_panel_mtype()
        ...
        neurons_panel_mtype_btn = outside_explore_page.find_neurons_mtypes_btn()
        ...
        if neurons_panel_mtype_btn and neurons_panel_mtype_btn.is_displayed():
            ...
            etype_title = outside_explore_page.find_neurons_etype_title()
        ...
        mtypes_neurons_panel = outside_explore_page.list_of_neurons_panel()
        logger.info(f"Neurons' panel with a list of M-types is found")

        panel_specific_mtype = outside_explore_page.find_neurons_panel_iso_mtype()
