...
from locators.simulate_paired_neurons_locators import SimulatePairedNeuronsLocators as Loc


class SimulatePairedNeuronsPage(HomePage):
    """Page object for the Paired neurons (beta) simulation page.

    Entry: Workflows → Simulate → Paired neurons → model picker → config → run → results.
    """
    ...
    def click_paired_neurons_card(self):
        """Click the Paired neurons card, scrolling the carousel if needed."""
        try:
            el = self.find_element(Loc.PAIRED_NEURONS_CARD, timeout=5)
            if el.is_displayed():
                el.click()
                self.logger.info("Clicked Paired neurons card (already visible)")
                time.sleep(5)
                return
        except TimeoutException:
            pass
        ...
        for _ in range(5):
            try:
                ...
                time.sleep(1)
                try:
                    el = self.find_element(Loc.PAIRED_NEURONS_CARD, timeout=3)
                    if el.is_displayed():
                        el.click()
                        self.logger.info("Clicked Paired neurons card (after scrolling)")
                        time.sleep(5)
                        return
                except TimeoutException:
                    continue
        ...
        raise RuntimeError("Paired neurons card not found after scrolling carousel")
    ...
    def _dismiss_tooltips(self):
        ...
        try:
            # Move mouse to top-left corner to unhover any tooltip triggers
        ...
        try:
            ...
            pass
        try:
            # Aggressively remove all tooltip/popover elements from DOM
            self.browser.execute_script("""
                ...
                // Also hide any element with very high z-index that looks like an overlay
    ...
    def _click_left_menu_btn(self, locator, label):
        ...
        # Scroll the left column to top so all menu buttons are reachable
        try:
            left_col = self.browser.find_element(
                By.CSS_SELECTOR, "div[class*='scrollable']"
            )
    ...
    # ── Dictionary items (shared by Stimuli, Recordings, Distributions, etc.) ──
    ...
    def click_dictionary_item_by_label(self, target_label):
        """Click a dictionary item by partial label match. Uses JS click to bypass overlays."""
        items = self.get_dictionary_items()
        assert items, "No dictionary items found"
        for item in items:
    ...
    def click_dictionary_item_by_label_with_scroll(self, target_label):
        """Scroll the middle column to find and click a dictionary item by label."""
        ...
        # Scroll the middle column down and retry
        try:
            scrollable = self.find_element(Loc.MIDDLE_COLUMN_SCROLLABLE, timeout=5)
            for _ in range(10):
                self.browser.execute_script(
                    "arguments[0].scrollTop += 200;", scrollable
                )
                time.sleep(0.5)
                try:
                    return self.click_dictionary_item_by_label(target_label)
    ...
    def set_block_number_input(self, label_substring, value):
        """Find a config block by label and set its number input value."""
    ...
    def get_input_file_buttons(self, timeout=10):
        try:
            ...
            names = [b.get_attribute("title") or "" for b in buttons]
    ...
    def is_plot_or_content_visible(self, timeout=10):
        """Check if any plot, canvas, or content is visible (for .h5 files)."""
        plot_locators = [
        ...
        for loc in plot_locators:
            try:
                el = self.find_element(loc, timeout=3)
                if el.is_displayed():
                    self.logger.info(f"Plot/content visible via {loc}")
                    return True
            except TimeoutException:
                continue
        return False
